...
|-------|-------|------|------|-----------|
| **The BotFather** (main) | `ollama/kimi-k2.6:cloud` | ?? Cloud | Executive Orchestrator | Persona-driven chat, memory, orchestration, direct takes |
| **Dapper Dan** | `ollama/kimi-k2.7-code:cloud` | ?? Cloud | Coding Specialist | Overnight builds, feature dev, token-efficient coding |
| **Breaking Ben** | `ollama/glm-5.2:cloud` | ?? Cloud | Testing/Breaking | Security audit, edge cases, stress tests, QA |
| **Codex Developer** | `ollama/glm-5.2:cloud` | ?? Cloud | Deep Development | Architecture, complex refactors, repo-scale analysis |
| **Chief of Staff** | `ollama/kimi-k2.7-code:cloud` | ?? Cloud | Coordination | Multi-agent routing, pipeline management, status reports |
...
Two options:
1?? Keep it - $250 (you own it)
2?? Free - I'll send you the files
...
### VV-20260712-093600 (July 12, 2026 — Latest Run)
| # | Suggestion | Impact | Status |
|---|-----------|--------|--------|
| 1 | Move C:\OpenClaw\obsbot-* to D:\camera\ (~4 GB) | Reclaim ~4 GB on C: | ? Pending approval |
| 2 | Clean npm cache (~2.3 GB) | Reclaim ~2 GB on C: | ? Pending approval |
| 3 | Archive/delete D:\backups\c-drive-cleanup (~3.68 GB) | Reclaim ~3.68 GB on D: | ? Pending approval |
| 4 | Clean C:\temp\quickslot-dev (~420 MB) | Reclaim ~0.4 GB on C: | ? Pending approval |
| 5 | Install 4 firmware/driver updates | System hygiene | ? No approval needed |
| 6 | Schedule quarterly elevated audit | Windows features inventory gap | ? Low priority |

### VV-20260712-091800 (July 12, 2026 — Earlier Run) — RESOLVED
| # | Suggestion | Impact | Status |
|---|-----------|--------|--------|
| 1 | Archive & remove C:\openclaw_backups (8.3 GB) | Reclaim ~8.3 GB on C: | ? DONE — removed |
| 2 | Clean npm cache (~2.3 GB) | Reclaim ~2 GB on C: | ? Still pending |
| 3 | Clean uv cache (~5.8 GB) | Reclaim ~5 GB on C: | ? DONE — cache cleaned |
| 4 | Install Defender update KB2267602 | Security hygiene | ? DONE — signature current |
| 5 | Schedule quarterly elevated audit | Windows features inventory gap | ? Low priority |

### VV-20260712-090000 (July 12, 2026 — Earlier Run) — RESOLVED
| # | Suggestion | Impact | Status |
|---|-----------|--------|--------|
| 1 | Clean npm cache (~6 GB) | Reclaim ~4-5 GB on C: | ? Still pending |
| 2 | Clean uv cache (~5.8 GB) | Reclaim ~5 GB on C: | ? DONE — cache cleaned |
| 3 | Review C:\openclaw_backups (10.44 GB) | Potential reclaim ~10 GB on C: | ? DONE — removed |
| 4 | Install Defender update KB2267602 | Security hygiene | ? DONE — installed |
| 5 | Schedule quarterly elevated audit | Windows features inventory gap | ? Low priority |

### VV-20260711 (Prior Run) — RESOLVED
| # | Suggestion | Status |
|---|-----------|--------|
| 1 | C: 18 GB drop (150?132 GB) | ? RESOLVED — Recovered to 167.8 GB |
| 2 | 5 pending updates | ? RESOLVED — Defender installed, 4 firmware/driver pending |


## ??? AI Engineering Platform v1.2.3 — Updated 2026-07-12

- **Package:** openclaw-ai-engineering-platform-selective v1.2.3
- **Mode:** Non-destructive overlay (no existing files modified)
- **Location:** workspace/platform/ai-engineering/
- **Git branch:** platform-overlay-pre-install-20260712-094024

### What Changed from v1.2 ? v1.2.3

**New Standards (3):**
- AGENT-PERSONALITY-STANDARD.md — behavioral profiles, phrase discipline, risk scaling
- ITALIAN-AMERICAN-LANGUAGE-STANDARD.md — phrase banks, profanity levels, authenticity rules
- BOTFATHER-DELEGATION-MODEL.md — delegation model, fast-lane rule, escalation rule

**New Prompts (2):**
- IMPLEMENT-PERSONALITY-PROFILES.md — behavioral profile installation guide
- IMPLEMENT-ITALIAN-AMERICAN-VOICE.md — Italian-American voice implementation guide

**New Agent Personalities (9 reference-only + 1 active upgrade):**
- Mikey Models (ACTIVE — upgraded model-buzz-scout with full personality)
- Tony Blueprints (REFERENCE-ONLY — product architecture)
- Bella Buttons (REFERENCE-ONLY — UX & interface design)
- Vinny Visuals (REFERENCE-ONLY — visual & brand review)
- Nico Stack (REFERENCE-ONLY — architecture & infrastructure)
- Joey No-Bugs (REFERENCE-ONLY — QA & testing)
- Sal the Shield (REFERENCE-ONLY — security review)
- Frankie Fastlane (REFERENCE-ONLY — performance)
- Rocco Rollout (REFERENCE-ONLY — release & deployment)
- Connie Consigliere (REFERENCE-ONLY — strategy & agent design)

**Each specialist has:** AGENT.md, SOUL.md, CURIOSITY.md, BEHAVIOR.md, PHRASES.md

**Updated:** model-buzz-scout agent upgraded to Mikey Models personality (BEHAVIOR.md + PHRASES.md added)

### What's Active
- ? **Mikey Models (Model Buzz Scout)** — weekly Sunday 8 AM ET cron, Italian-American personality, delivers to Telegram
- ? **weekly-model-intelligence skill** — installed
- ? **14 platform skills** — product-discovery through weekly-model-intelligence
- ? **11 engineering + personality standards**
- ? **4 core agent bindings** — BotFather, Breaking Ben, Chief of Staff, Codex Developer

### What's Reference-Only (Not Registered)
- ?? 9 Italian-American specialists — reference-on-demand, BotFather delegates when needed
- ?? 10 v1.2 specialists — reference-only (art-director, product-architect, etc.)

### What's Deferred
- ?? Capability Evolution monthly cron — not scheduled
- ?? Automatic specialist agent registration — waiting for approval

### Delegation Model
BotFather remains sole primary orchestrator. Specialists invoked only when expertise materially improves outcome. Fast-lane rule: small/reversible/low-risk changes need no specialist review. Escalation: use multiple specialists only when task crosses disciplines.

### Rollback
- Full: Remove-Item -Recurse -Force workspace/platform/ai-engineering/ + revert model-buzz-scout agent + remove cron + git checkout
- Partial: Remove specialist personalities only, keep standards and Mikey Models
